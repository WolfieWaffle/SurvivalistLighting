...
public class BlockHardcoreTorch extends BlockTorch {
		...
		if (world.getBlockState(pos).getBlock() instanceof BlockHardcoreTorch) {
			if (!heldItem.isEmpty() && (heldItem.getItem() == Items.FLINT_AND_STEEL/* || (ConfigHandler.matchboxCreatesFire && heldItem.getItem() == HardcoreTorchesItems.matchbox)) && (!ConfigHandler.noRelightEnabled || !isLit()*/)) {
				...
				if (!world.isRainingAt(pos)) {
					world.setBlockState(pos, getState(world, pos, ModBlocks.torchLit), 2);
				}
	...
	public void updateTorch(World world, BlockPos pos) {
		if (canBurnout() && world.isRainingAt(pos) && isLit()) {
			//world.setBlockState(pos, getState(world, pos, RealisticTorchesBlocks.torchBurnt), 2);
			extinguish(world, pos, true);
		} else if (canBurnout() && isLit() && ((Math.random() * ConfigHandler.torchBurnoutChance) < 1)) {
			//world.setBlockState(pos, getState(world, pos, RealisticTorchesBlocks.torchBurnt), 2);
			extinguish(world, pos, true);
		}
	}
